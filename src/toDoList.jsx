  ...
  return (
    <div className="container">
      ...
      {toDoList.map((toDo, index) => (
        <div key={`todo-${index}-${toDo.text}`}>
          <input type="checkbox" checked={toDo.checked} onChange={() => toggleCheck(index)} />
          <span style={{ textDecoration: toDo.checked ? 'line-through' : 'none' }}>
            {toDo.text}
          </span>
          <button type="button" onClick={() => deleteToDoList(index)}>
            삭제
          </button>
        </div>
      ))}
      <div>
        <button type="button" onClick={deleteAllToDoList}>
          전체 삭제
        </button>
      </div>
    </div>
